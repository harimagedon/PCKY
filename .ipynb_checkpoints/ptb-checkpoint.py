...
def tag_parent(tx):
    def proc(tx, st):
        if tx.leaf():
            ...
            #print(type(tx.parent().symbol().label),tx.parent().symbol().label)
    traverse(tx, proc)
...
def remove_comma(node):
    ...
    def pre(node, state):
        delete = []
        for i,child in enumerate(node.children()):
            if child.leaf() and child.leaf().pos == ',':
                delete.append(i)
...
def remove_quotation(node):
    ...
    def pre(node, state):
        ...
        for i,child in enumerate(node.children()):
            if child.leaf() and child.leaf().pos == '``':
                delete.append(i)
            elif child.leaf() and child.leaf().pos == '\'\'':
                delete.append(i)
...
def generate_terminal_rules(tx):
    def pre(tx, st):
        if tx.leaf():
            return st + [tx.rule()]
        return st
